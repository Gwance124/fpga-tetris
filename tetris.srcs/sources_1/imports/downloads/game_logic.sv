...
gwance124 — 4/3/25, 1:51 AM
assign reg_addr = 20*DrawY[9:4] + DrawX[9:5];
Mang-0 — 4/3/25, 1:51 AM
what did we change in thatone
gwance124 — 4/3/25, 1:51 AM
but with this
// assign reg_addr = {char_y, char_x[6:2]}; //30 rows 20 cols 600 total
Mang-0 — 4/3/25, 1:52 AM
okok
gwance124 — 4/3/25, 1:52 AM
yaya
Mang-0 — 4/3/25, 1:52 AM
are you only changing that line
gwance124 — 4/3/25, 1:52 AM
yeah
Mang-0 — 4/3/25, 1:52 AM
ok
...
so i think its just this
Mang-0 — 4/3/25, 1:52 AM
okokokokoKOKOKOKO
gwance124 — 4/3/25, 1:52 AM
Mang-0 — 4/3/25, 1:53 AM
pls
gwance124 — 4/3/25, 2:21 AM
LFGGGGG
RAAHHHH
Mang-0 — 4/3/25, 2:22 AM
YAYAYAAYAHAHAHAHAHAHAH
RAHHHHHHHHH
WE DID ITTTTTTTTT
gwance124 — 4/3/25, 2:22 AM
Mang-0 — 4/3/25, 2:23 AM
we gon sleep good tn
gwance124 — 4/3/25, 2:23 AM
Oh we for shore wiggling our feet
Bruh someone is snoring in the comp lab
Iz time for squid game
Mang-0 — 4/8/25, 5:02 PM
`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
...
message.txt
15 KB
this one i tried moving BRAM into the axi sv
`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// Company: ECE-Illinois
// Engineer: Zuofu Cheng
...
module  color_mapper ( 
    ...
    output  logic [3:0] FGD_idx, BGD_idx;
    output  logic       pixel_invert;
    

    logic [6:0] char_x; 

    logic [15:0] char_data_with_invert;
    logic [6:0] char_code;
    logic invert_bit;

    logic pixel_bit;

    assign char_x = DrawX[9:3]; //80
    assign pixel_bit = font_data[7 - DrawX[2:0]];

    always_comb begin
        case (char_x[0])
            1'b0: char_data_with_invert = regs[15:0];
            1'b1: char_data_with_invert = regs[31:16];
        endcase
    end

    assign char_code = char_data_with_invert[14:8];
    assign invert_bit = char_data_with_invert[15];
    assign font_addr = {char_code, DrawY[3:0]};
...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    uint32_t temporary;
        ...
        if (color % 2) {
            ...
            temporary |= (red << 20) | (green << 16) | (blue << 12);
        } else {
            temporary &= 0xFFFFE001;
            temporary |= (red <<  8) | (green <<  4) | (blue <<  0);
        }
        hdmi_ctrl->palette[color / 2] = temporary;
}
struct TEXT_HDMI_STRUCT {
    uint8_t  VRAM[ROWSCOLUMNS2];
    uint8_t  skip[PALETTE_START - (ROWSCOLUMNS2)];
        uint32_t palette[8];
}; 
Mang-0 — 4/10/25, 4:14 PM
`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// Company: ECE-Illinois
// Engineer: Zuofu Cheng
...
message.txt
5 KB
gwance124 — 4/10/25, 5:00 PM
Image
Mang-0 — 4/10/25, 5:17 PM
if (addra[13] == 1)
        begin
...
message.txt
4 KB
gwance124 — 4/10/25, 9:13 PM
bobasdjfi23
gwance124 — 4/10/25, 11:18 PM
Attachment file type: acrobat
lab72saucetypeshit.pdf
188.86 KB
gwance124 — 4/23/25, 10:42 PM
```
module tetris_top (
    input logic Clk,
...
message.txt
3 KB
gwance124 — 4/24/25, 9:13 PM
localparam logic [35:0] TETROMINO_SHAPES [0:6][0:3] = '{
        // 0: I-Tetrimino (Cyan)
        '{
            // State 0:
            {4'd1, 5'd0, 4'd1, 5'd1, 4'd1, 5'd2, 4'd1, 5'd3},
            // State 1:
Expand
message.txt
3 KB
https://github.com/educ8s/Cpp-Tetris-Game-with-raylib/blob/main/src/blocks.cpp
GitHub
Cpp-Tetris-Game-with-raylib/src/blocks.cpp at main · educ8s/Cpp-Te...
C++ Tetris Game with raylib with a video tutorial explaining each line. - educ8s/Cpp-Tetris-Game-with-raylib
C++ Tetris Game with raylib with a video tutorial explaining each line. - educ8s/Cpp-Tetris-Game-with-raylib
Mang-0 — 4/24/25, 9:35 PM
localparam logic [35:0] TETROMINO_SHAPES [0:6][0:3] = '{
        // 0: I-Tetrimino (Cyan)
        '{
            // State 0:
...
message.txt
4 KB
gwance124 — 4/24/25, 10:01 PM
`timescale 1ns/1ps

module game_logic_tetromino_tb;
...
message.txt
6 KB
gwance124 — 5/3/25, 7:03 PM
Code with line clear
